cat <<'EOF' > /usr/local/bin/cka
#!/usr/bin/env bash
# ==============================================================================
# CKA Exam Simulator CLI
# Usage:
...
2. Decide which policy allows interaction between frontend and backend deployments
   in the least permissive way and deploy that yaml to the backend namespace.
QEOF
;;
    14) cat <<'QEOF'
...
grade_q2() {
  ...
  if [[ -s "$manifest_file" ]]; then
    ...
    if [[ "$crd_lines" -eq 0 ]] && [[ "$argo_deploy" -gt 0 ]]; then
      file_valid=1
    fi
  fi

  if [[ -n "$helm_repo" ]] && [[ "$ns_exists" -ge 1 ]] && [[ "$file_valid" -eq 1 ]]; then
...
grade_q4() {
  ...
  if [[ "$replicas" -eq 3 ]] && [[ "${ready:-0}" -eq 3 ]] && [[ -n "$c_req_cpu" ]] && \
...
grade_q5() {
  ...
  if [[ "$target" == "apache-deployment" ]] && [[ "$min_r" -eq 1 ]] && [[ "$max_r" -eq 4 ]] && \
     [[ "$util" -eq 50 ]] && [[ "$stab" -eq 30 ]]; then
...
grade_q9() {
  ...
  if [[ "$svc_status" == "active" ]] && [[ "$p1" -eq 1 ]] && [[ "$p2" -eq 1 ]] && \
     [[ "$p3" -eq 1 ]] && [[ "$p4" -ge 131072 ]]; then
...
grade_q12() {
  ...
  if [[ "$svc_type" == "NodePort" ]] && [[ "$svc_port" -eq 8080 ]] && \
...
grade_q15() {
  ...
  if [[ "$api_healthy" -ge 1 ]] && [[ -z "$bad_port" ]]; then
...
grade_q16() {
  ...
  if [[ "$c_name" == "http" ]] && [[ "$s_type" == "NodePort" ]] && [[ "$s_nodeport" -eq 30080 ]]; then
...
grade_q17() {
  ...
  if [[ -n "$hosts_line" ]] && [[ ! "$cm_tls" =~ "TLSv1.2" ]] && \
     [[ "$tls12_rejected" -eq 1 ]] && [[ "$tls13_accepted" -eq 1 ]]; then
